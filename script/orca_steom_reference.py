...
def build_orca_input(xyz_path, basis, n_states):
...
def parse_orca_steom_output(text):
    ...
    Returns: list of (state_index, omega_Ha, omega_eV, sample_character_string)
    """
    results = []
    ...
    for line in lines:
        ...
        if stripped.startswith("IROOT=") and " au " in line and " eV " in line:
            try:
                ...
                results.append((idx_state, omega_Ha, omega_eV, ""))
            except (ValueError, StopIteration, IndexError):
                pass
    return results


def main(xyz_path, basis, n_states, dry_run=False):
    inp_text = build_orca_input(xyz_path, basis, n_states)
    ...
    print(f"   state  omega (Ha)         omega (eV)    character")
    for (idx, oHa, oeV, char) in states:
        print(f"    {idx:3d}   {oHa:.10f}     {oeV:8.4f}   {char.strip()[:50]}")
...
if __name__ == "__main__":
    ...
    main(xyz, basis, n_states, dry_run=dry_run)
