#!/usr/bin/env python3
"""Local (WSL g++) numerical test of include/steom_gphph_projection_full.inc:
dump H2O arrays + Python-reference FULL-CLASS routes, compile a host-only
harness that includes the .inc verbatim, run, compare (all element classes).

Run:  wsl python3 script/steom_gphph_incfull_test.py
"""
...
int main(int argc, char** argv) {
    ...
    printf("[incfull test] route_ph max|d| = %.3e   route_mc max|d| = %.3e  (expect <=1e-12)\n", dph, dmc);
...
def main():
    ...
    Ms, Mc = CGF.parse_json()
    route_ph = GEN.subtract(Mc, Ms)
    ref_ph = GEN.evaluate(route_ph, arrays, nocc, nvir)
    ref_mc = GEN.evaluate(Mc, arrays, nocc, nvir)
