#!/bin/bash
# Benchmark: Stored vs Direct (half-transform) vs Hash (half-transform) for ADC(2)
# Usage: cd build && bash ../script/benchmark_half_transform_adc2.sh
...
echo "================================================================"
echo "  Half-Transform ADC(2) Benchmark"
echo "  Stored (full nao^4) vs Direct (half-transform) vs Hash (half-transform)"
echo "================================================================"
echo ""

declare -a TESTS=(
    "$XYZ_DIR/H2O.xyz:$BASIS_DIR/cc-pvdz.gbs:H2O/cc-pVDZ:3"
    "$XYZ_DIR/Benzene.xyz:$BASIS_DIR/sto-3g.gbs:Benzene/STO-3G:3"
    "$XYZ_DIR/Benzene.xyz:$BASIS_DIR/cc-pvdz.gbs:Benzene/cc-pVDZ:3"
    "$XYZ_DIR/Naphthalene.xyz:$BASIS_DIR/sto-3g.gbs:Naphthalene/STO-3G:3"
    "$XYZ_DIR/Naphthalene.xyz:$BASIS_DIR/cc-pvdz.gbs:Naphthalene/cc-pVDZ:3"
)
...
run_bench() {
    ...
    _time=$(echo "$out" | grep "ADC(2) time:" | head -1 | awk '{print $(NF-1)}')
...
printf "%-22s %4s %4s %4s  %8s %7s  %8s %7s  %8s %7s  %10s %10s %10s\n" \
    "System" "nao" "nocc" "nvir" "Stor(s)" "Mem MB" "Dir(s)" "Mem MB" "Hash(s)" "Mem MB" "E1(Ha)" "E2(Ha)" "E3(Ha)"
printf "%s\n" "--------------------------------------------------------------------------------------------------------------------------------------"
...
for test in "${TESTS[@]}"; do
    ...
    echo -n " stored" >&2
    run_bench -x "$xyz" -g "$basis" --post_hf_method ADC2 --n_excited_states "$nst"
    nao=$_nao; nocc=$_nocc; nvir="?"
    [[ "$nao" != "?" && "$nocc" != "?" ]] && nvir=$((nao - nocc))
    t_stored=$_time; m_stored=$_mem; e1s=$_e1; e2s=$_e2; e3s=$_e3

    echo -n " direct" >&2
    run_bench -x "$xyz" -g "$basis" --eri_method Direct --post_hf_method ADC2 --n_excited_states "$nst"
    [[ "$nao" == "?" ]] && nao=$_nao
    [[ "$nocc" == "?" ]] && nocc=$_nocc
    [[ "$nao" != "?" && "$nocc" != "?" ]] && nvir=$((nao - nocc))
    ...
    echo -n " hash" >&2
    run_bench -x "$xyz" -g "$basis" --eri_method hash --post_hf_method ADC2 --n_excited_states "$nst"
    [[ "$nao" == "?" ]] && nao=$_nao
    [[ "$nocc" == "?" ]] && nocc=$_nocc
    [[ "$nao" != "?" && "$nocc" != "?" ]] && nvir=$((nao - nocc))
...
echo "  Times = ADC(2) solver only (excl. SCF), Mem = GPU peak (incl. SCF + ERI)"
