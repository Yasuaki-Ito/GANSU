...
// ---------------------------------------------------------------------------
//  ROHF Pipek-Mezey localized occupied MOs → Molden file
//  Doubly-occupied (1..num_beta_spins) and singly-occupied (num_beta_spins+1
//  ..num_alpha_spins) blocks are localized SEPARATELY so PM doesn't mix the
//  closed-shell core with open-shell electrons. Virtual block keeps canonical.
// ---------------------------------------------------------------------------
void ROHF::export_lmo_molden_file(const std::string& filename) {
    std::ofstream ofs(filename);
    if (!ofs) {
        THROW_EXCEPTION("Failed to open the file: " + filename);
    }
    ...
    ofs << "[Molden Format]" << std::endl;
    ofs << "[Title]" << std::endl;
    ofs << "generated by GANSU (ROHF LMO export)" << std::endl;
    ...
    const int n_docc = static_cast<int>(num_beta_spins);                  // doubly-occupied
    const int n_socc = static_cast<int>(num_alpha_spins - num_beta_spins); // singly-occupied
    ...
    // Localize one occupied sub-block [col_offset, col_offset + n) of coefficient_matrix
    auto localize_block = [&](int col_offset, int n_block) -> DLPNOLocalizationResult {
        DLPNOLocalizationResult empty{};
        if (n_block <= 0) return empty;
        std::vector<real_t> C_occ(static_cast<size_t>(nao) * n_block);
        for (int mu = 0; mu < nao; ++mu)
            for (int i = 0; i < n_block; ++i)
                C_occ[static_cast<size_t>(mu) * n_block + i] =
                    coefficient_matrix(mu, col_offset + i);
        return localize_pipek_mezey(
            C_occ.data(), S.data(),
            nao, n_block, atom_ao_ranges,
            /*max_sweep=*/200, /*conv_tol=*/1e-10, /*verbose=*/1);
    };

    auto loc_docc = localize_block(0, n_docc);
    auto loc_socc = localize_block(n_docc, n_socc);

    auto eps_lmo_diag = [&](const DLPNOLocalizationResult& loc, int col_offset, int n_block) {
        std::vector<real_t> eps(n_block, 0.0);
        for (int i = 0; i < n_block; ++i) {
            real_t s = 0.0;
            for (int k = 0; k < n_block; ++k) {
                const real_t u = loc.U[static_cast<size_t>(k) * n_block + i];
                s += u * u * orbital_energies[col_offset + k];
            }
            eps[i] = s;
        }
        return eps;
    ...
    for (int i = 0; i < n_docc; ++i) {
        ofs << "Sym= A" << std::endl;
        ofs << "Ene= " << eps_docc_lmo[i] << std::endl;
        ofs << "Spin= Alpha" << std::endl;
        ofs << "Occup= 2.0" << std::endl;
        for (int mu = 0; mu < nao; ++mu)
            ofs << " " << mu+1 << " " << std::setprecision(17)
                << loc_docc.C_LMO[static_cast<size_t>(mu) * n_docc + i] << std::endl;
    }
    // singly-occupied LMOs
    for (int i = 0; i < n_socc; ++i) {
        ofs << "Sym= A" << std::endl;
        ofs << "Ene= " << eps_socc_lmo[i] << std::endl;
        ofs << "Spin= Alpha" << std::endl;
        ofs << "Occup= 1.0" << std::endl;
        for (int mu = 0; mu < nao; ++mu)
            ofs << " " << mu+1 << " " << std::setprecision(17)
                << loc_socc.C_LMO[static_cast<size_t>(mu) * n_socc + i] << std::endl;
    }
    // canonical virtuals
    for (int i = static_cast<int>(num_alpha_spins); i < nao; ++i) {
        ofs << "Sym= A" << std::endl;
        ofs << "Ene= " << orbital_energies[i] << std::endl;
        ofs << "Spin= Alpha" << std::endl;
    ...
    ofs.close();
    std::cout << "[LMO Molden] ROHF: " << n_docc << " doubly-occ LMOs ("
              << loc_docc.n_sweeps << " sweeps) + " << n_socc
              << " singly-occ LMOs (" << loc_socc.n_sweeps
              << " sweeps) + " << (nao - n_docc - n_socc)
              << " canonical virtuals → " << filename << std::endl;
}
