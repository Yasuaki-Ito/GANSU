...
void Rys_compute_gradients_two_electron_cpu(double* g_gradients, const real_t* g_density_matrix, const PrimitiveShell* g_shell, const real_t* g_cgto_normalization_factors, const ShellTypeInfo shell_s0, const ShellTypeInfo shell_s1, const ShellTypeInfo shell_s2, const ShellTypeInfo shell_s3, const size_t num_threads, const int num_basis, const double* g_boys_grid)
{
    ...
    // --- Component loop ---
    for (int ia = 0; ia < na; ia++) {
        int l1 = loop_to_ang_host[la][ia][0], m1 = loop_to_ang_host[la][ia][1], n1 = loop_to_ang_host[la][ia][2];
        double NA = calcNorm(alpha, l1, m1, n1);

        for (int ib = 0; ib < nb; ib++) {
            int l2 = loop_to_ang_host[lb][ib][0], m2 = loop_to_ang_host[lb][ib][1], n2 = loop_to_ang_host[lb][ib][2];
            double NB = calcNorm(beta, l2, m2, n2);

            for (int ic = 0; ic < nc; ic++) {
                int l3 = loop_to_ang_host[lc][ic][0], m3 = loop_to_ang_host[lc][ic][1], n3 = loop_to_ang_host[lc][ic][2];
                double NC = calcNorm(gamma_e, l3, m3, n3);
    ...
    // Write to global gradient
    for (int dir = 0; dir < 3; dir++) {
        gansu_atomic_add(&g_gradients[3*sa.atom_index + dir], grad_atom[0+dir]);
        gansu_atomic_add(&g_gradients[3*sb.atom_index + dir], grad_atom[3+dir]);
        gansu_atomic_add(&g_gradients[3*sc.atom_index + dir], grad_atom[6+dir]);
        gansu_atomic_add(&g_gradients[3*sd.atom_index + dir], grad_atom[9+dir]);
    }
    }
}

// CPU host-callable mirror of Rys_compute_gradients_two_electron_2pdm.  Same body as the GPU
// kernel; the per-thread loop is an OpenMP parallel for that
// assigns `id` directly.  Auto-generated.
...
// CPU host-callable mirror of Rys_compute_gradients_two_electron_uhf.  Same body as the GPU
// kernel; the per-thread loop is an OpenMP parallel for that
// assigns `id` directly.  Auto-generated.
void Rys_compute_gradients_two_electron_uhf_cpu(double* g_gradients, const real_t* g_density_matrix_alpha, const real_t* g_density_matrix_beta, const PrimitiveShell* g_shell, const real_t* g_cgto_normalization_factors, const ShellTypeInfo shell_s0, const ShellTypeInfo shell_s1, const ShellTypeInfo shell_s2, const ShellTypeInfo shell_s3, const size_t num_threads, const int num_basis, const double* g_boys_grid)
{
    #pragma omp parallel for schedule(dynamic)
    for (long long id_ll = 0; id_ll < (long long)num_threads; id_ll++) {
        const size_t id = (size_t)id_ll;
    ...
    for(int ia_c=0;ia_c<na;ia_c++){
        ...
        for(int ib_c=0;ib_c<nb;ib_c++){
            ...
            for(int ic_c=0;ic_c<nc;ic_c++){
                ...
                double NC=calcNorm(gamma_e,l3,m3,n3);
                for(int id_cc=0;id_cc<nd;id_cc++){
                    int l4=loop_to_ang_host[ld][id_cc][0],m4=loop_to_ang_host[ld][id_cc][1],n4=loop_to_ang_host[ld][id_cc][2];
                    double ND=calcNorm(delta_e,l4,m4,n4);
