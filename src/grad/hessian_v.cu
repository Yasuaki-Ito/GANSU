...
// CPU host-callable mirror of compute_hessian_nuclear_attraction.  Auto-generated.
            ...
            for (int lmn_b = 0; lmn_b < comb_max(b.shell_type); lmn_b++) {
                ...
                for (int d1 = 0; d1 < 3; d1++) {
                    for (int d2 = 0; d2 < 3; d2++) {
                        ...
                        // AA block
                        gansu_atomic_add(&g_hessian[(3*iA+d1)*ndim+(3*iA+d2)], vAA);
                        // BB block
                        gansu_atomic_add(&g_hessian[(3*iB+d1)*ndim+(3*iB+d2)], vBB);
                        // AB block
                        gansu_atomic_add(&g_hessian[(3*iA+d1)*ndim+(3*iB+d2)], vAB);
                        // BA block = transpose of AB
                        gansu_atomic_add(&g_hessian[(3*iB+d1)*ndim+(3*iA+d2)], vBA);

                        // AC block: d²V/(dA_d1 dC_d2) = -(d²V/dA_d1 dA_d2 + d²V/dA_d1 dB_d2)
                        double vAC = -(vAA + vAB);
                        gansu_atomic_add(&g_hessian[(3*iA+d1)*ndim+(3*iC+d2)], vAC);
                        // CA block = transpose
                        gansu_atomic_add(&g_hessian[(3*iC+d2)*ndim+(3*iA+d1)], vAC);

                        // BC block: d²V/(dB_d1 dC_d2) = -(d²V/dB_d1 dA_d2 + d²V/dB_d1 dB_d2)
                        double vBC = -(vBA + vBB);
                        gansu_atomic_add(&g_hessian[(3*iB+d1)*ndim+(3*iC+d2)], vBC);
                        // CB block = transpose
                        gansu_atomic_add(&g_hessian[(3*iC+d2)*ndim+(3*iB+d1)], vBC);

                        // CC block: d²V/(dC_d1 dC_d2) = AA + AB + BA + BB
                        double vCC = vAA + vAB + vBA + vBB;
                        gansu_atomic_add(&g_hessian[(3*iC+d1)*ndim+(3*iC+d2)], vCC);
                    }
                }
            }
        }
