...
// CPU host-callable mirror of compute_hessian_overlap.  Auto-generated.
void compute_hessian_overlap_cpu(double* g_hessian, const real_t* g_W_matrix, const PrimitiveShell* g_shell, const real_t* g_cgto_normalization_factors, const int num_basis, const int num_atoms, ShellTypeInfo shell_s0, ShellTypeInfo shell_s1, const size_t num_threads)
{
    ...
    for (int d1 = 0; d1 < 3; d1++) {
        for (int d2 = d1; d2 < 3; d2++) {
            ...
            // AA block
            gansu_atomic_add(&g_hessian[gA1 * ndim + gA2], val);
            if (d1 != d2) gansu_atomic_add(&g_hessian[gA2 * ndim + gA1], val);

            // BB block (same as AA)
            gansu_atomic_add(&g_hessian[gB1 * ndim + gB2], val);
            if (d1 != d2) gansu_atomic_add(&g_hessian[gB2 * ndim + gB1], val);

            // AB block (negative of AA)
            gansu_atomic_add(&g_hessian[gA1 * ndim + gB2], -val);
            if (d1 != d2) gansu_atomic_add(&g_hessian[gA2 * ndim + gB1], -val);

            // BA block (negative of AA)
            gansu_atomic_add(&g_hessian[gB1 * ndim + gA2], -val);
            if (d1 != d2) gansu_atomic_add(&g_hessian[gB2 * ndim + gA1], -val);
        }
    }
    }
}
