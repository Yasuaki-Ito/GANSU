...
# Run Hartree-Fock
# =============================================================================
def run_hf(geometry, basis, spin):
    mol  = pyscf.M(atom=geometry, basis=basis, spin=spin)
    myhf = mol.RHF().run()
    return myhf
...
if __name__ == '__main__':
    ...
    h1e, eri_new, norb, nelec, ecore = build_integral(myhf)
    end_integral = time.time()
    if rank == 0 and args.debugmode > 0:
        print(f"pre_time: {end_integral - start_hf:.3f} s \n")
    # Run FCI
    e_value, na, nelec_t   = run_fci(h1e, eri_new, norb, nelec, ecore, E_HF, geometry, args)
