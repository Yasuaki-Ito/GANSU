 ...
 * For JCTC reviewers only. Not for commercial use without permission.
 */
#include <cuda.h>
#include <cub/cub.cuh>
...
void contract_2e_spin1_gpu_buf_overlap(
    ...
    double         *d_ci0,          double         *d_ci1,
    double         *e_check,
    int32_t        *maxtona,        int32_t        *natomax,
...
__global__
void _convertids(int32_t *maxtona, int32_t *natomax, int32_t *starts_aid, int32_t*ends_aid, int32_t na, int32_t na_max_total, int32_t na_max_node, int rank)
{
    int32_t thread_id = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (id_in_proc < local_count) {
        ...
        // Use a more appropriate sentinel value
        maxtona[thread_id] = -1;
    }
}
    ...
    cudaStream_t computeStream)
{
    ...
    if (cfg.nprocs > 1) {
        for (int i = 0; i < 2; i++) {
            CUDA_CHECK(cudaMalloc((void**)&dl_t1[i],  t1_size));
            CUDA_CHECK(cudaMalloc((void**)&dl_vt1[i], t1_size));
        }
        ...
        CUDA_CHECK(cudaMalloc((void**)&maxtona, na_max_total  * sizeof(int32_t)));
        ...
        _convertids<<<nblock_conv, min(nthread, na_max), 0, computeStream>>>(
            maxtona, natomax, d_starta, d_enda,
    ...
    for (int icyc = 0; icyc < cfg.max_cycle; icyc++) {
        ...
        if (reset_state == 1) {
            ...
            contract_2e_spin1_gpu_buf_overlap(
                ...
                d_ci0, d_ci1, e_check, maxtona, natomax,
                cfg, pc, computeStream, commStream, ncclComm);
        } else {
            contract_2e_spin1_gpu(
                handle, eri, d_ci0, d_ci1, d_t1, d_vt1,
    ...
    double   tol,        double   E_rhf)
{
    ...
    const size_t t1_size     = (size_t)my_na  * chunk * nnorb      * sizeof(double);
    ...
    double  *d_ci1       = NULL, *d_vt1       = NULL, *d_t1       = NULL;
    ...
    CUDA_CHECK(cudaMalloc((void**)&d_t1,        t1_size));
    CUDA_CHECK(cudaMalloc((void**)&d_vt1,       t1_size));
...

    {
        ...
        davidson(
            ...
            d_ci0, d_ci1, d_t1, d_vt1, d_eri, d_hdiag, hdiag,
...
cleanup:
    ...
    safe_free_dev((void*&)d_t1);
    safe_free_dev((void*&)d_vt1);
